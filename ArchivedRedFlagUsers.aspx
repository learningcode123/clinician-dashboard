...
<% sServiceTitle = "Archived Users" %>

<%
Dim oDataTable_RedFlags As System.Data.DataTable

If UserIsCounselor() Then
	oDataTable_RedFlags = ExecuteDataTableSP("_DashboardData","GetArchivedRedFlagUsers", nUserID, 0 , "counselor")
Else
	oDataTable_RedFlags = ExecuteDataTableSP("_DashboardData","GetArchivedRedFlagUsers", nUserID, 0 , "admin")
End If
...
If nNumUsers > 0 Then	'Apply Settings to RedFlagUsers
	oDataTable_RedFlags = ApplySettings_RedFlags(oDataTable_RedFlags, nUserID, 2)
End If
 ...
 <div class="form-item">
  ...
  <%
		...
		For Each kvp As KeyValuePair(Of Integer, Date) In user_redpin_info_MaxDate_dict
			 ...
			 If Row("SurveyScore") = 2 Then 'BAM Score Decline			
			...
			If Row("MedicationSurvey") = 2 Then 'Medication Survey			
			 		redpinColumn_Code = GetRedpinColumn_Code("SurveyScore")			 		
			...
			If Row("InactiveUser2") = 2 Then	'InActive User	
			 ...
			 If Row("DidUse") = 2 Then	'Used alcohol or drugs
			 ...
			 If Row("NaltroxoneInjection") = 2 Then	'Missed Naltrexone Injection	
